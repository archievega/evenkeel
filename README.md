...
## Seeing which guard fired

Every guard above answers the client with the same `503`. The dashboard is where
they stop looking alike.

![The evenkeel dashboard while its one dependency is deliberately broken](docs/img/dashboard.png)

**Read the seconds on that picture as a property of the stub, not of this
service.** The run behind it points the API at a provider configured to stall
for 700ms with a concurrency cap of 32, then offers it 200 writes a second —
about ten times what that path can complete. Nine calls in ten are shed
immediately and the rest sit against the retry budget, which is what the tail
in the latency panel is. The same code with no provider in the way serves the
same load in tens of milliseconds. Both runs, and the caveats that make either
number nearly meaningless on a laptop, are in
[tools/load/README.md](tools/load/README.md).

The picture exists because a degraded dependency is the only condition under
which the middle row has anything to say.

```bash
make observe   # api + postgres + prometheus + grafana, on localhost:3000
make load      # 200 writes/s for 30s against the stalling provider
```

It is regenerated by `make dashboard-image` rather than pasted from a laptop — a
screenshot that cannot be reproduced is a claim, and this repository does not
ship those.

Eight alert rules sit alongside it in
[ops/prometheus/alerts.yml](ops/prometheus/alerts.yml), under one constraint:
**none of them may fire on an idle service.** Every ratio carries a minimum
volume gate, because `0/0` is not the problem — one request, one failure and a
ratio of `1.0` is. The rules were ported from a production service whose Redis
memory alert divided by a `maxmemory` nobody had set, paged on a healthy
instance, and taught everyone to ignore the channel.

The dashboard is checked against the code by
`tests/unit/test_observability_stack.py`: every panel and every alert must name
a metric the adapter actually emits, and no scrape label may shadow one the
application sets. That last test exists because `service: evenkeel` on the
target looked like harmless decoration and quietly replaced the label naming the
*dependency* — so `by (service)` grouped by a constant and drew one line for
every provider. The panels still rendered.
...
`/metrics` with a Grafana dashboard and alert rules over them, the MCP
transport, JWT/JWKS verification against any OIDC issuer, and the OpenAPI
contract with its drift and breaking-change checks.

Not started, and deliberately not listed above as if they were: tracing, an
outbox, a worker. The reasoning behind what is here
lives in `docs/adr/`.
