...
![The evenkeel dashboard under a degraded provider](docs/img/dashboard.png)

```bash
make observe   # api + postgres + prometheus + grafana, on localhost:3000
make load      # 200 writes/s for 30s against a provider stalling at 700ms
```

That picture is one run of exactly those two commands: 176 calls/s refused by
the concurrency cap against 7.5/s getting through, while request latency holds
at 2.4s and the ledger stays correct. It is regenerated by `make
dashboard-image` rather than pasted from a laptop — a screenshot that cannot be
reproduced is a claim, and this repository does not ship those.
