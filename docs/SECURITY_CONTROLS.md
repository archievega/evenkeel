...
| The signing algorithm comes from configuration; the token's own `alg` header is never consulted | [`adapters/jwt/identity.py`](../src/evenkeel/infrastructure/adapters/jwt/identity.py) | `test_an_algorithm_outside_the_configured_allowlist_is_refused`, `test_a_key_published_for_encryption_cannot_verify_a_signature`, `test_an_alg_none_token_is_refused`, `test_an_hmac_token_signed_with_the_public_key_is_refused`, `test_a_token_signed_by_a_different_key_is_refused`, `test_a_token_without_a_key_id_is_refused_without_a_fetch` | [347](https://cwe.mitre.org/data/definitions/347.html) |
